...
$result = mysql_query("select rfc,empresa,
	concat(nombre_contacto,' ',apellido_paterno,' ',apellido_materno) as nombre,
	concat(calle,' ',numero,' ',colonia,'') as direccion,ciudad,estado,telefono,celular,email
						from clientes 
						where status = 'ACTIVO'  ");

	while ($datatmp = mysql_fetch_array($result)) {
	    $data[] = array_merge($datatmp, array('rfc'));
	}
	$options = array(
		...
		'fontSize'=>8 ,
		'xPos'=>'center',
	);	$titles = array('rfc'=>'<b>RFC</b>',
						'empresa'=>'<b>EMPREESA</b>',
						'nombre'=>'<b>NOMBRE</b>',
...
$pdf->addText(200,740,12,"<b>Listado Clientes </b>\n"); 
