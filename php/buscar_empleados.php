<?php 
 ...
 conectarse();
  ...
  if (!$se) {
    $sql = "select matricula,nombre,apellido_paterno,apellido_materno,calle,numero,colonia,ciudad,estado,telefono,celular,sueldo,tipo,status from empleados  where  status = 'ACTIVO'";
  ...
  $sql = "select matricula,nombre,apellido_paterno,apellido_materno,calle,numero,colonia,ciudad,estado,telefono,celular,sueldo,tipo,status from empleados".$se;
   if( !empty($post['orden']) && !empty($post['orderby']))
    //Añadimos de una ves la parte de la consulta para ordenar el resultado
    $sql .= " ORDER BY $post[orderby] $post[orden] ";
  if($post['limit'] && $post['offset']) $sql.=" limit $post[offset], $post[limit]";
  ...
  $i = 0;
     ...
     while($row = mysql_fetch_object($query)){
      ...
      $result[$i]['cell'] = array($row->matricula,$row->nombre,$row->apellido_paterno,
      $row->apellido_materno,$row->calle,$row->numero,$row->colonia,$row->ciudad,$row->estado,
      $row->telefono,$row->celular,$row->sueldo,$row->tipo,$row->status);
     $i++;
    }     
