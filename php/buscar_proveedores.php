...
//Creamos un arreglo con los datos que envia JqGrid
  ...
  if($post['search'] == 'true'){
    ...
    $search['like']=elements(array('nombre','contacto'),$_REQUEST);
    ...
    $search['where'] = elements(array('nombre','contacto'),$_REQUEST);
  ...
  if (!$se) {
    $estado = 'activo';
    $sql = "select clave_proveedor,nombre,contacto,observaciones,direccion,colonia,telefono,celular,email,estado from proveedores  where  estado = 'ACTIVO'";
  ...
  $sql = "select clave_proveedor,nombre,contacto,observaciones,direccion,colonia,telefono,celular,email,estado from proveedores".$se;
    if( !empty($post['orden']) && !empty($post['orderby']))
    //Añadimos de una ves la parte de la consulta para ordenar el resultado
    $sql .= " ORDER BY $post[orderby] $post[orden] ";
  if($post['limit'] && $post['offset']) $sql.=" limit $post[offset], $post[limit]";
  ...
  $i = 0;
     ...
     while($row = mysql_fetch_object($query)){
       ...
       $result[$i]['cell'] = array($row->clave_proveedor,$row->nombre,$row->contacto,$row->observaciones,$row->direccion,$row->colonia,$row->telefono,$row->celular,$row->email,$row->estado);
     $i++;
    }     
